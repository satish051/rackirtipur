...
let content = fs.readFileSync('index - Copy - Copy.html', 'utf8');
...
// 3. Testimonial
const testimonialRegex = /"The dedication of the Kirtipur Rotaractors transformed our children's home\. Their library project didn't just fill empty shelves; it opened up new worlds for hundreds of students\."[\s\S]*?Chairperson, COTS Nepal<\/p>\s*<\/div>\s*<\/div>\s*<\/div>\s*<\/div>/;
...
// 4. Members Modal
const membersRegex = /<!-- Board of Directors -->[\s\S]*?<\/div>\s*<\/div>\s*<\/div>\s*<\/div>\s*<!--/i;
const membersReplacement = `<!-- Board of Directors -->
                <div class="mb-8">
                    <h4 class="text-rota-gold font-bold uppercase tracking-widest text-xs mb-4 flex items-center">
                        <span class="w-6 h-[1px] bg-rota-gold mr-2"></span> Board of Directors
                    </h4>
                    
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <% members.filter(m => m.isBoardMember).forEach(m => { %>
                        <div class="flex items-center p-3 bg-gray-800/50 rounded-xl border border-gray-700/50 hover:bg-gray-800 transition-colors">
                            <div class="w-12 h-12 rounded-full bg-rota-cranberry/20 flex items-center justify-center text-rota-cranberry font-bold mr-4 border border-rota-cranberry/30">
                                <%= m.name.charAt(0) + (m.name.split(' ')[1] ? m.name.split(' ')[1].charAt(0) : '') %>
                            </div>
                            <div>
                                <p class="text-white font-medium text-sm"><%= m.name %></p>
                                <p class="text-rota-cranberry text-xs font-semibold uppercase tracking-wider mt-0.5"><%= m.role %></p>
                            </div>
                        </div>
                        <% }) %>
                    </div>
                ...
                <!-- General Members -->
                <div>
                    <h4 class="text-gray-400 font-bold uppercase tracking-widest text-xs mb-4 flex items-center">
                        <span class="w-6 h-[1px] bg-gray-600 mr-2"></span> General Members
                    </h4>
                    
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                        <% members.filter(m => !m.isBoardMember).forEach(m => { %>
                        <div class="flex items-center p-2.5 bg-gray-800/30 rounded-lg border border-gray-700/30">
                            <div class="w-8 h-8 rounded-full bg-gray-800 flex items-center justify-center text-gray-400 font-bold mr-3 text-xs">
                                <%= m.name.charAt(0) + (m.name.split(' ')[1] ? m.name.split(' ')[1].charAt(0) : '') %>
                            </div>
                            <div>
                                <p class="text-gray-200 text-sm font-medium"><%= m.name %></p>
                                <p class="text-gray-500 text-[10px] uppercase tracking-wider"><%= m.role %></p>
                            </div>
                        </div>
                        <% }) %>
                    </div>
            ...
            <!-- Modal Footer -->
            <div class="px-6 py-5 bg-gray-900 border-t border-gray-800 flex justify-end sticky bottom-0">
                <button onclick="closeProjectModal('members-modal')" class="px-6 py-2 bg-gray-800 hover:bg-gray-700 text-white font-semibold rounded-lg transition-colors border border-gray-700">
                    Close
                </button>
            </div>
        </div>
    ...
    <!--`;
content = content.replace(membersRegex, membersReplacement);

fs.writeFileSync(file, content, 'utf8');
console.log('Successfully generated EJS!');
